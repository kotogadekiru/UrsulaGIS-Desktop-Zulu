...
public class MeasureTool extends AVListImpl implements Disposable {
    ...
    public void setMeasureShape(SurfaceShape surfaceShape) {
        ...
        } else // SurfacePolygon, SurfacePolyline, SurfaceSector, or some custom shape
        {
            ...
            // Extract positions from shape
            updatePositionsFromShape();//read from the shape into positions and innerBoundaries
            // Create control points for each position except the last that is the same as the first
            int index = 0;
            for (int i = 0; i < this.positions.size() - 1; i++) {
                addControlPoint(this.positions.get(i), CONTROL_TYPE_LOCATION_INDEX, index);
                index++;
            }
            for(List<Position> ib : innerBoundaries){
                for(int i=0;i<ib.size()-1;i++){
                    addControlPoint(ib.get(i), CONTROL_TYPE_LOCATION_INDEX, index);
                    index++;
                }
            }
        }
    ...
    public void moveControlPoint(ControlPoint point, String mode) {
        ...
        if (point.getValue(CONTROL_TYPE_LOCATION_INDEX) != null) {
            ...
            setPositionAtIndex(positionIndex, surfacePosition);
    ...
    /** search for the position at the given index including inner boundaries */
    ...
    /** set the position at the given index including inner boundaries */
    public void setPositionAtIndex(int positionIndex, Position surfacePosition){
        if(positionIndex < positions.size() - 1){//evita el ultimo porque es igual al primero
            positions.set(positionIndex, surfacePosition);
            if(positionIndex==0){
    ...
    protected void updateMeasureShape() {
        ...
        if (this.measureShapeType.equals(SHAPE_LINE) || this.measureShapeType.equals(SHAPE_PATH)) {
            ...
            if (this.surfaceShape != null) {
                ...
                //XXX set inner boundaries
                for(List<Position> ib : innerBoundaries){
                    ((SurfacePolygon) this.surfaceShape).addInnerBoundary(ib);
                }                
            }
    ...
    protected void updatePositionsFromShape() {
        ...
        if (this.surfaceShape instanceof SurfacePolygon) {
            SurfacePolygon surfacePolygon = (SurfacePolygon) this.surfaceShape;
            for(int i=1;i<surfacePolygon.getBoundaries().size();i++){
                Iterable<? extends LatLon> innerBoundary = surfacePolygon.getBoundaries().get(i);
                
                List<Position> innerBoundaryList = new ArrayList<Position>();
                innerBoundary.forEach(latLon -> innerBoundaryList.add(new Position(latLon, 0)));
                this.innerBoundaries.add(innerBoundaryList);
            }             
        }
    }
...
//        this.controlPointsLayer.removeAllRenderables(); // TODO: why commented out? Are annotations being disposed?
    ...
    protected void addControlPoint(Position position, String indexKey, Object indexValue) {
        ControlPoint controlPoint = new ControlPoint(new Position(position, 0), this.controlPointsAttributes, this);
        controlPoint.setValue(indexKey, indexValue);
